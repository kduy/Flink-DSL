...
// select 
  ...
  // union/except/intersect  ~> NOT USE
...
/*// create
lazy val createStmt = "create table".i ~ ident ~ optParens(typedColums) ^^^ Create[Option[String]]()

lazy val typedColums = rep1sep(typedColum)
lazy val typedColum  = named ~ typed */
...
object Test extends  SqlParser{

  def parser :(SqlParser, String) => ?[Ast.Statement[Option[String]]] = (p : SqlParser, s:String) => p.parseAllWith(p.stmt, s)
  ...
  def main(args: Array[String]) {
   ...
   /* println (parseAllWith(stmt, " select * from person where age = 10 and name = ?"))
    println("-------"*10)
    println (parseAllWith(stmt, " select * from (select age from person) as p"))
    println("-------"*10)
    println (parseAllWith(stmt, " select p.age    , j.type from person p join job j on p.job = j.id"))
    println("-------"*10)
    println (parseAllWith(stmt, " select p.age    , j.type from person p join job j "))
*/
